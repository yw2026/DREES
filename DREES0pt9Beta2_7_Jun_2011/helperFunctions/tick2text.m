...
Options = struct('xformat', [], 'yformat', [], 'zformat', [], ...
                 ...
                 'ztickoffset', .08, 'axis', 'xy', 'panon', true, 'color',[0 0 0]);
...
texttick(ax, xformatfun, yformatfun, zformatfun, Options.ytickoffset, Options.xtickoffset, Options.ztickoffset, Options.axis, Options.color);
...
function texttick(haxis, xformat, yformat, zformat, ytickoffset, xtickoffset, ztickoffset, tickax, tickColor)
...
if ~isempty(strfind(tickax, 'x'))
    ...
    if isappdata(haxis, 'XTickText')
        ...
        delete(hxold);
        hx = text(xtick, xlabely, xlabelz, xticklab, paramval{:}, 'parent', haxis, 'color', tickColor);      
        setappdata(haxis, 'XTickText', hx);

    else
        hx = text(xtick, xlabely, xlabelz, xticklab, 'horizontalalignment', 'center', 'parent', haxis, 'color', tickColor);      
        setappdata(haxis, 'XTickText', hx);
    end
end
...
if ~isempty(strfind(tickax, 'y'))
    ...
    if isappdata(haxis, 'YTickText')
        ...
        delete(hyold);
        hy = text(ylabelx, ytick, ylabelz, yticklab, paramval{:}, 'parent', haxis, 'color', tickColor);      
        setappdata(haxis, 'YTickText', hy);

    else
        hy = text(ylabelx, ytick, ylabelz, yticklab, 'horizontalalignment', 'center', 'parent', haxis, 'color', tickColor);
        setappdata(haxis, 'YTickText', hy);
    end
end
...
if ~isempty(strfind(tickax, 'z'))
    ...
    if isappdata(haxis, 'ZTickText')
        ...
        delete(hzold);
        hz = text(zlabelx, zlabely, ztick, zticklab, paramval{:}, 'parent', haxis, 'color', tickColor);      
        setappdata(haxis, 'ZTickText', hz);

    else
        hz = text(zlabelx, zlabely, ztick, zticklab, 'horizontalalignment', 'center', 'parent', haxis, 'color', tickColor);
        setappdata(haxis, 'ZTickText', hz);
    end
end

rmappdata(haxis, 'XTickText');
rmappdata(haxis, 'YTickText');
% rmappdata(haxis, 'ZTickText');
